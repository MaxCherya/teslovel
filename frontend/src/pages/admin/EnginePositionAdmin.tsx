...
const EnginePositionAdmin: React.FC = () => {
    ...
    useEffect(() => {
        // Fetch engine positions (replace with real API call, e.g., /api/motor-positions/)
        setEnginePositions([
            { id: 1, name_uk: "Передній мотор", name_en: "Front Hub", name_ru: "Передний мотор" },
            { id: 2, name_uk: "Середній мотор", name_en: "Mid-Drive", name_ru: "Средний мотор" },
            { id: 3, name_uk: "Задній мотор", name_en: "Rear Hub", name_ru: "Задний мотор" },
        ]);
    }, []);

    const handleRemove = (id: number) => {
        // Remove engine position (replace with real API call)
        setEnginePositions((prev) => prev.filter((position) => position.id !== id));
        console.log(`Removing engine position with id: ${id}`);
    };
                            ...
                            <tbody>
                                {enginePositions.map((position) => (
                                    <tr key={position.id} className="border-t border-gray-200">
                                        <td className="px-2 sm:px-4 lg:px-6 py-3 sm:py-4 text-xs sm:text-sm lg:text-base">{position.name_uk}</td>
                                        <td className="px-2 sm:px-4 lg:px-6 py-3 sm:py-4 text-xs sm:text-sm lg:text-base">{position.name_en}</td>
                                        <td className="px-2 sm:px-4 lg:px-6 py-3 sm:py-4 text-xs sm:text-sm lg:text-base">{position.name_ru}</td>
